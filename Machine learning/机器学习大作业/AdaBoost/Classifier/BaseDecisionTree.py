...
class BaseDecisionTree:
    ...
    def fit(self, X, y, sample_weight=None):
         ...
         for i in range(n):
            ...
            # fea_min=np.min(feature)
            # fea_max=np.max(feature)
            # stepsize=(fea_max-fea_min)/stepnum
            # for j in range(-1,stepnum+1):
            #      thres = fea_min+stepsize*j  # 逐一设定可能阈值
            fea_unique = np.sort(np.unique(feature))  # 将所有特征值从小到大排序
            for j in range(len(fea_unique)-1):
                ...
                for op in (0, 1):
                     ...
                     if err < self.lowest_error:  # 当前参数组合可以获得更低错误率，更新最优参数
                         self.lowest_error = err
                         self.best_op = op
                         self.best_fea_id = i
